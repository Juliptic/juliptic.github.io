...
We expect this action to preserve rational structures and since we understand that the action comes from an associated matrix called the stark regulator matrix, then our conjecture shall be that we can write down explcit linear combinations of cohomology classes over $$\mathbb{C}$$ which are asscoaited to $$f$$, which when scaleed by untis should land in $$H^i(X,\omega)_f\otimes E$$. 
